...
public class AnbubingobookModVariables {
	...
	public static class MapVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AnbubingobookMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				AnbubingobookMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "anbubingobook_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AnbubingobookMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				AnbubingobookMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					AnbubingobookMod.PACKET_HANDLER.sendToAll(message);
				else
					AnbubingobookMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
